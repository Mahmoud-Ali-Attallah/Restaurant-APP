...
function show(x){
    ...
    for(let i = 0 ;  i < x.categories.length ; i++){
        ...
        <div  class="con d-flex flex-column justify-content-center position-absolute p-4 rounded rounded-2 bg-body-tertiary  " style="--bs-bg-opacity: .8;">
        <p class ="text-black it mt-4 mb-0  fs-2 fw-bold">${x.categories[i].strCategory}</p>
        <p class ="text-black mt-1 mb-4 text.center fs-6 ">${x.categories[i].strCategoryDescription.slice(0,150)}</p>
           </div>
        <img src=${x.categories[i].strCategoryThumb} class=" rounded rounded-2 d-block " alt="${x.categories[i].strCategory}" width="100%">
    ...
    function clic(){
        ...
        let z = document.querySelectorAll(".this .con .it")
        console.log(z)
    ...
    async function Country(x){
        let respone = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${x}`) ;
        let data = await respone.json() ;
        await showNew(data) ; 
    }
    function showNew(x){
        ...
        for(let i = 0 ;  i < x.meals.length ; i++){
            container+=` <div class="this col-sm-12  col-xl-6  col-xxl-3  text-white position-relative overflow-hidden text-center mb-2 ">
            <img src=${x.meals[i].strMealThumb} class=" rounded rounded-2 d-block " alt="" width="100%">
            <div  class="con  d-flex flex-row align-items-center justify-content-cenetr  position-absolute pb-4  mx-2 rounded rounded-2 bg-body-tertiary  " style="--bs-bg-opacity: .8;">
                <p class ="text-black fs-4 fw-bold">${x.meals[i].strMeal}</p>
               </div>
